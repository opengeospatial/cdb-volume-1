...
The schema of vector attributes can be found in `Vector_Attributes.xsd` file stored in the schema folder of the CDB datastore (`CDB\Metadata\Schema\Vector_Attributes.xsd`) and accessible from the (http://schemas.opengis.net/cdb/1.1/Vector_Attributes.xsd)[Official Schemas].
The list of CDB attributes are described in `CDB_Attributes.xml` file stored in the metadata folder of the CDB datastore (`\CDB\Metadata\CDB_Attributes.xml`) and accessible from the (http://schemas.opengis.net/cdb/1.3/Metadata/CDB_Attributes.xml)[Official Schemas].
...
| Attribute Code
| A unique four-digit numeric code is associated to each attribute.

| Attribute Symbol (Identifier)
| A unique three-character or four-character alphanumeric identifier is associated to the attributes that are governed by this standard.

| Attribute Values
| A value can be assigned to each attribute. Attribute values give quantitative/qualitative meaning to the attribute. The data type, enumeration, length, format, range, usage, units, compatibility and schema of each attribute value is governed by this standard.
...
This section provides a list and definition of the attributes specific to the CDB standard and not from external standards.
