...
The following diagram is the UML Model for CDB Attribute schema and defining different entities and their properties, together with the relationship between entities.
...
*_{figure-caption}{counter:figure-num}. The UML diagram of the CDB vector attribute_*

The UML diagram comprises three major sections (i.e., attributes, units and scalers).
The main properties of the vector attributes’ UML diagram are documented below. A complete listing and description of CDB attributes are provided in the `CDB_Attributes.xml` file of this standard. The file is described in Section 5.1.8 of Volume 1: CDB Attributes Metadata.
...
| Attribute Code
| A unique four-digit numeric code is associated with each attribute.
...
List of CDB attributes and description of the attributes that are governed by the CDB standard provided in `\CDB\Metadata\CDB_Attributes.xml`. Note that it is possible to provide attributes other than those listed here by making use of the Geomatics and Vendor Extended-level attribution schema.

CDB Attributes are stored in the XML format which is more appropriate for a computer program. The schema can be found in `CDB\Metadata\Schema\Vector_Attributes.xsd`.
