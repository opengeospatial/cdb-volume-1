...
The UML diagram was generated based on the `Vector_Attributes.xsd` file and comprised three sections (i.e., attributes, units, and scalers).
Attributes have two main properties as named by Level and Value. The main properties of the vector attributes are included in the Value (i.e., data type, enumeration, length, format, range, usage, units, compatibility).
...
| Attributes are used to describe one or more real or virtual characteristics of a feature. Attributes have three characteristics: +
  - _Code_: A unique four-digit numeric code is associated with each attribute. +
  - _Symbol_ (Identifier): A unique three-character or four-character alphanumeric identifier is associated with the attributes that are governed by this standard. +
  - _Deprecated_: It states if the attribute is Deprecated or not.
