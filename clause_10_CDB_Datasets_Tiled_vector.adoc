...
| Attributes are used to describe one or more real or virtual characteristics of a feature. Attributes have three characteristics: +
  - _Attribute Code_, A unique four-digit numeric code is associated with each attribute.
  - _Attribute Symbol_ (Identifier): A unique three-character or four-character alphanumeric identifier is associated with the attributes that are governed by this standard. +
