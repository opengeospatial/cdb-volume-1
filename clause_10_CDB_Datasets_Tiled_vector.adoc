...
The UML diagram was generated based on the `Vector_Attributes.xsd` file and comprised three sections: attributes, units, and scalers.
...
|Number of Vertices (NVT)|Number of vertices of the 3D model associated with the point feature.
|Damage Level (DAMA)|Represents the level of damage to the feature and its model.
...
2+|*Category IV) Application Schema Geometry Attributes:*
...
To make the above table machine  readable, allocation table is converted to an XML code in the OGC CDB v1.3 Standard.
The table is provided in the CDB_Attributes.xml file (\CDB\Metadata\CDB_Attributes.xml) as a new ComplexType called 'Allocation' added to the "Attribute" element.
In order to adopt this change, the Vector_Attributes.xsd file (\CDB\Metadata\Schema\Vector_Attributes.xsd) is also updated.
These changes are described in the OGC CDB V1.3 volume 10.
