...
|001 |003 |*.tif |Primary Alternate Terrain Elevation |A grid of data representing the Elevation of the surface of the Earth at specified Latitude and Longitude offsets inside each grid element. Stored as a 4-channel TIFF image.  Deprecated in CDB 1.3
...
There are two component selectors and methods of storing the constituents of the Primary Alternate Terrain Elevation.  Both methods store the elevation, mesh type, and the specified latitude and longitude offsets inside each grid element.  One stores the constituents as a 4 channel TIFF image, and the other stores the data as a TIFF image with 3 separate raster grids with the same dimensions.  The second method is preferred due to greater compatibility with existing software.
