...
|001 |004 |*.tif |Primary Alternate Terrain Elevation |A grid of data representing the Elevation of the surface of the Earth at specified Latitude and Longitude offsets inside each grid element. Stored as 3 individual raster grids within a single TIFF image.  _Preferred alternate to the 4-channel TIFF option._
...
The Primary Alternate Terrain Elevation dataset is composed of three pieces of data for each grid element: the elevation value, the mesh type code, and the offset values that locate the elevation value within the grid element.  The two different Primary Alternate Terrain Elevation datasets store the same data, but use a different structure and data encoding.  The different encodings are discussed below.

The elevation data and the mesh type are defined in the same way as the Primary Elevation dataset.  The offset values are pairs of values that represent the latitude and longitude offset values. These values provide positional offsets within the grid spacing for the LOD in question. Note that since the movement of each elevation point is constrained to the inside of its respective grid element, it is impossible to disrupt the (regular grid) topology of the elevation grid. Furthermore, moving elevation points outside the confines of the tile-LOD is impossible. Figure 5-10 illustrates the valid offset values inside each grid element of a tile-LOD.
...
The _Latitude Offset_ can be expressed as either an 8-bit unsigned integer value ranging from 0 to 255, or as a 32-bit floating point value ranging from 0.0 to 1.0 (not including 1.0). For the 8-bit data type, the value is scaled so that each grid element is split into 256 equal parts in the latitude direction. For the floating point data type, the value is scaled so that 0.0 is the original latitude and 1.0 is the next grid latitude. Thus, the elevation point cannot be positioned on the latitude of the next grid element directly north of the current grid element.

The _Longitude Offset_ can be expressed either as an 8-bit unsigned integer value ranging from 0 to 255, or as a 32-bit floating point value ranging from 0.0 to 1.0 (not including 1.0). For the 8-bit data type, the value is scaled so that each grid element is split into 256 equal parts in the longitude direction. For the floating point data type, the value is scaled so that 0.0 is the original longitude and 1.0 is the next grid longitude. Thus, the elevation point cannot be positioned on the longitude of the next grid element directly east of the current grid element.

There are two different methods (with different component selectors) provided to store the constituents of the Primary Alternate Terrain Elevation.  Both methods store the elevation, mesh type, and the specified latitude and longitude offsets inside each grid element.  However, each method stores the data in a different manner.

The first method stores the data elements as a 4 channel TIFF image.  The first channel is the elevation value stored as either a signed binary or a floating point value as in the Primary Elevation dataset.  The second channel is the mesh type stored as an 8-bit unsigned binary value with a 0 or 1 value.  The third and fourth channels are unsigned 8-bit latitude and longitude offsets, respectfully. This data format is not recommended due to incompatibility with common TIFF libraries that do not support TIFF images with different channel sizes.

The second method stores the data elements as a single TIFF image with 3 subfile raster grids with the same dimensions. The first subfile is the elevation data stored as a single channel image using either a signed binary or floating point values as in the Primary Elevation dataset.  The second subfile is the mesh type stored as a single channel image using either an 8-bit binary or a 1-bit Bilevel image.  The third subfile is the latitude and longitude offsets stored as a two channel image using either 8, 16, or 32 bit unsigned binary or 32-bit floating point values.  If the Primary Alternate Elevation dataset is used, this data encoding is recommended because it has better compatibility with existing image software.
...
If the Primary Alternate Terrain Elevation dataset is not available (files associated with the Primary Alternate Terrain Elevation component for the area covered by a tile, at a given LOD or coarser, are either missing or cannot be accessed), a CDB client-device should use the Primary Terrain Elevation dataset.  In the absence of missing _Mesh Type_, _Latitude Offset_, or _Longitude Offset_ values, they are assumed to be zero.
