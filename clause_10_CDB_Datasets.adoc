...
Note that the highest accuracy offset values for the _Latitude Offset_ and _Longitude Offset_ values are 32-bit unsigned integers.
...
The first method stores the data elements as a 4 channel TIFF image.  The first channel is the elevation value stored as either a signed integer or a floating point value as in the Primary Elevation dataset.  The second channel is the mesh type stored as an 8-bit unsigned integer value with a 0 or 1 value.  The third and fourth channels are unsigned 8-bit integer latitude and longitude offsets, respectively. Note:  this data format is not recommended due to incompatibility with common TIFF libraries that do not support TIFF images with different channel sizes.
