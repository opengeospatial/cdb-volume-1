...
Since each GSModel can have more than one level of detail, discovery of the associated GSModelDescriptor is needed.  Therefore, there must be a GSModelDescriptor file for each model LOD, stored at the same CDB LOD that each LOD of the model is stored at.
