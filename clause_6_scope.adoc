...
* *RGB*: The RGB format. Included in the Gamma Tutorial Section, Volume 10: OGC CDB Implementation Guidance
...
* *GeoPackages*: The normative requirements and conformance classes for the use of GeoPackages in a CDB data store. The GeoPackage content encoding rules documented in Volume 13: OGC CDB - Rules for Structuring a GeoPackage;
* *JPEG*: JPEG 2000 file format (Volume 2: OGC CDB Core Model and Physical Structure Annexes, Annex H).

Each of these documents has been annotated to reflect the conventions established by the CDB standard. The Best Practice conventions currently define how TIFF, OpenFlight, RGB, Shapefile and JPEG 2000 formatted files are to be interpreted by CDB-compliant simulator readers.
