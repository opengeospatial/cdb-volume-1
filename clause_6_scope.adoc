...
See /CDB/Metadata/Feature_Data_Dictionary.xml for the complete list of the supported codes footnote:[Currently, these are a mixture of FACC, SEDRIS, DGIWG, DO-272B, UHRB, and other codes, Future revisions of the CDB standard shall provide guidance on using other feature code lists.].
...
* *SGI image format*: A native raster graphics file format also known as RGB file format.
* *Shapefiles*: The Best Practice use of the Shapefile format in a CDB data store. The Shapefile table content encoding rules are in Volume 4: OGC CDB Best Practice use of Shapefiles for Vector Data Storage;
