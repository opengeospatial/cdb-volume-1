...
The CDB attributes and the metadata for these attributes are stored in `\CDB\1.3\Metadata\CDB_Attributes.xml` and the schema can be found in `\CDB\1.3\Vector_Attributes.xsd`. The file is CDB Attributes into a format more appropriate for a computer program.
