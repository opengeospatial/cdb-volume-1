...
6.  GeoPackage (*.gpkg): used for the representation and attribution of vector data. Refer to Volume 13, OGC CDB Rules for Structuring a GeoPackage, for details.
...
Previous version sf the above table had a column labeled: _CDB Client-device Behavior for Prior Versions". All rows had the label _Ignores data_. The column has been removed but the value is still valid.
